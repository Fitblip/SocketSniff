...
class SymbolikEmulator:
    ...
    def __init__(self, vw):
        ...
        self._sym_expr_help = ExpressionHelper()
    ...
    def parseExpression(self, expr, update=True):
        sym = self._sym_expr_help.parseExpression(expr)
        if update:
    ...
    def readSymMemory(self, symaddr, symsize):
        ...
        if self._sym_vw != None:
            ...
            # Otherwise, if it has a name, return the name symbol
            sym = self._sym_vw.getSymByAddr(addrval, exact=True)
            if sym != None:
                return Var(str(sym), self.__width__)
    ...
    def setSymVariable(self, name, symval, width=None):
        ...
        symval = frobSymbol(symval, width)
