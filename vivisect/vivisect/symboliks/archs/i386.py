...
class IntelSymbolikTranslator(vsym_trans.SymbolikTranslator):
    ...
    def getRegObj(self, regidx):
        ...
        if ridx != regidx:
            ...
            if lshift != 0:
                val >>= Const(lshift, rbitwidth / 8)

            val = val & Const(mask, rbitwidth / 8)
    ...
    def getOperAddrObj(self, op, idx):
        ...
        if isinstance(oper, e_i386.i386RegMemOper):
            ...
            if oper.disp == 0:
                return reg
            if oper.disp > 0:
                return o_add(reg, Const(oper.disp, self._psize), self._psize)
            if oper.disp < 0:
                return o_sub(reg, Const(abs(oper.disp), self._psize), self._psize)
    ...
    def i_neg(self, op):
        ...
        self.setOperObj(op, 0, o_sub(Const(0, self._psize), obj, self._psize))
    ...
    def i_not(self, op):
        v1 = self.getOperObj(op, 0)
        v1 ^= Const(e_bits.u_maxes[v1.getWidth()], self._psize)
    ...
    def i_setnz(self, op):
        self.setOperObj(op, 0, cnot(Var('eflags_eq', self._psize)))

    def i_setz(self, op):
        self.setOperObj(op, 0, Var('eflags_eq', self._psize))
...
class i386SymbolikTranslator(IntelSymbolikTranslator):
    ...
    def i_jecxz(self, op):
        return self._cond_jmp(op, eq(Var('ecx', self._psize), Const(0, self._psize)))

    def i_leave(self, op):
        self.effSetVariable('esp', Var('ebp', self._psize))
        self.effSetVariable('ebp', Mem(Var('esp', self._psize), Const(4, self._psize)))
        self.effSetVariable('esp', o_add(Var('esp', self._psize), Const(4, self._psize), self._psize))
    ...
    def i_stosd(self, op):
        ...
        edi = Var('edi', self._psize)
        self.effWriteMemory(edi, Const(4, self._psize), Var('eax', self._psize))
        # FIXME flags?
