...
class ForceLayout(vg_layouts.GraphLayout):
    ...
    def _shiftGraphLayout(self, graph, dx, dy):
        ...
        [ graph.setNodeInfo(n, 'position', (p['position'][0] + dx, p['position'][1] + dy) ) for n,p in nodes ]
    ...
    def _setEdgePoints(self):
        for eid,n1,n2,einfo in self.graph.getEdges():
            n1pos = self.graph.getNodeInfo(n1,'position')
            n2pos = self.graph.getNodeInfo(n2,'position')
